...
${analysis.gamblingAnalysis.significantTransactions.map(t => 
  `<tr><td>${t.date}</td><td>${t.description}</td><td class="negative">₦${t.outflow.toLocaleString()}</td><td>₦${t.inflow.toLocaleString()}</td><td class="negative">₦${(t.outflow - t.inflow).toLocaleString()}</td></tr>`
).join('')}
</table>
...
${analysis.salaryAnalysis.salaryTransactions.map(s => 
  `<tr><td>${s.date}</td><td>₦${s.amount.toLocaleString()}</td><td>${s.employer}</td></tr>`
).join('')}
</table>
<table>
<tr><th>Metric</th><th>Value</th></tr>
<tr><td>Total Salary Income</td><td>₦${analysis.salaryAnalysis.totalSalaryIncome.toLocaleString()}</td></tr>
<tr><td>Average Monthly Salary</td><td>₦${analysis.salaryAnalysis.averageMonthlySalary.toLocaleString()}</td></tr>
...
${analysis.outflowAnalysis.expenditureBreakdown.map(cat => 
  `<tr><td>${cat.category}</td><td>₦${cat.amount.toLocaleString()}</td><td>${cat.percentage}%</td><td>${cat.transactions.map(t => `${t.description}: ₦${t.amount.toLocaleString()}`).join(', ')}</td></tr>`
).join('')}
</table>
